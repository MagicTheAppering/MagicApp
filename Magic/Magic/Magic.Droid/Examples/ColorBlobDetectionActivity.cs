...
namespace Magic.Droid.Examples
{
    ...
    public class ColorBlobDetectionActivity : Activity, View.IOnTouchListener, CameraBridgeViewBase.ICvCameraViewListener2
    {
        ...
        {
            Log.Info("BlobDetection", "Instantiated new " + GetType().ToString());
        }
        ...
        protected override void OnResume()
        {
            ...
            {
                Log.Debug("BlobDetection", "Internal OpenCV library not found. Using OpenCV Manager for initialization");
                OpenCVLoader.InitAsync(OpenCVLoader.OpencvVersion300, this, mLoaderCallback);
            }
            else
            {
                Log.Debug("BlobDetection", "OpenCV library found inside package. Using it!");
        ...
        public override void OnManagerConnected(int status)
        {
            switch (status)
            {
                case LoaderCallbackInterface.Success:
                    {
                        Log.Info("BlobDetection", "OpenCV loaded successfully");
